...
angular.module('ngGrid.services').factory('$domUtilityService',['$utilityService', '$window', function($utils, $window) {
    ...
    domUtilityService.BuildStyles = function($scope, grid, digest) {
        ...
        var directionHtml = "body[dir='ltr']";
        if (document.body.dir == "rtl") {
            directionHtml = "body[dir='rtl']";
        }
        css = directionHtml+" ." + gridId + " .ngCanvas { width: " + trw + "px; } " +
            directionHtml + " ." + gridId + " .ngRow { width: " + trw + "px; } " +
            directionHtml + " ." + gridId + " .ngCanvas { width: " + trw + "px; } " +
            directionHtml + " ." + gridId + " .ngHeaderScroller { width: " + (trw + domUtilityService.ScrollH) + "px} ";
        ...
        for (var i = 0; i < cols.length; i++) {
            ...
            if (col.visible !== false) {
                if (document.body.dir == "rtl") {
                    css += directionHtml + " ." + gridId + " .col" + i + " { width: " + col.width + "px; right: " + sumWidth + "px; height: " + rowHeight + "px } " +
                        directionHtml + " ." + gridId + " .colt" + i + " { width: " + col.width + "px; } ";
                } else {
                    css += directionHtml + " ." + gridId + " .col" + i + " { width: " + col.width + "px; left: " + sumWidth + "px; height: " + rowHeight + "px } " +
                        directionHtml + " ." + gridId + " .colt" + i + " { width: " + col.width + "px; } ";
                }
                sumWidth += col.width;
            }
    ...
    domUtilityService.setColLeft = function(col, colLeft, grid) {
        if (grid.styleText) {
            ...
            if (!regex) {
                if (document.body.dir == "rtl") {
                    regex = regexCache[col.index] = new RegExp(".col" + col.index + " { width: [0-9]+px; right: [0-9]+px");
                } else {
            ...
            var css;
            if (document.body.dir == "rtl") {
                css = grid.styleText.replace(regex, ".col" + col.index + " { width: " + col.width + "px; right: " + colLeft + "px");
            } else {
                css = grid.styleText.replace(regex, ".col" + col.index + " { width: " + col.width + "px; left: " + colLeft + "px");
            }
            domUtilityService.setStyleText(grid, css);
        }
    };
...
var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
    ...
    self.onMouseMove = function(event) {
        var diff = 0;
        if (document.body.dir == "rtl") {
            diff = self.startMousePosition - event.clientX;
        } else {
            diff = event.clientX - self.startMousePosition;
        }
...
var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
    ...
    $scope.adjustScrollLeft = function (scrollLeft,clientWidth, scrollWidth) {
        if (!clientWidth) {
            clientWidth = 0;
        }
        if (!scrollWidth) {
            ...
            if (col.visible !== false) {
                ...
                if (col.pinned) {
                    ...
                    if (document.body.dir != "rtl") {
                        ...
                        if (domUtilityService.getIEVer() > 0) {
                            rtlScrollLeft = Math.abs(scrollLeft);
                        } else {
                            rtlScrollLeft = scrollWidth - clientWidth - scrollLeft;
                        }
                        newLeft = i > 0 ? (rtlScrollLeft + totalLeft) : rtlScrollLeft;
                        totalLeft += col.width;
                    }
...
angular.module('ngGrid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('footerTemplate.html',
    ...
    "    </div>\r" +
    "\n" +
    "    <div class=\"ngPagerContainer\" style=\"float: " + directionRight + "; margin-top: 10px;\" ng-show=\"enablePaging\" ng-class=\"{'ngNoMultiSelect': !multiSelect}\">\r" +
    "\n" +
    "        <div style=\"float: " + directionLeft + "; margin-" + directionRight + ": 10px;\" class=\"ngRowCountPicker\">\r" +
    "\n" +
    "            <span style=\"float: " + directionLeft + "; margin-top: 3px;\" class=\"ngLabel\">{{i18n.ngPageSizeLabel}}</span>\r" +
    "\n" +
    "            <select style=\"float: " + directionLeft + ";height: 27px; width: 100px\" ng-model=\"pagingOptions.pageSize\" >\r" +
    "\n" +
    ...
    "        </div>\r" +
    "\n" +
    "        <div style=\"float:" + directionLeft + "; margin-" + directionRight + ": 10px; line-height:25px;\" class=\"ngPagerControl\" style=\"float: " + directionLeft + "; min-width: 135px;\">\r" +
    "\n" +
  ...
  $templateCache.put('headerRowTemplate.html',
    ...
    "\t<div class=\"ngVerticalBar\" ng-style=\"{height: col.headerRowHeight}\" ng-class=\"{ ngVerticalBarVisible: true }\">&nbsp;</div>\r" +
    "\n" +
  ...
  $templateCache.put('rowTemplate.html',
    ...
    "\t<div class=\"ngVerticalBar\" ng-style=\"{height: rowHeight}\" ng-class=\"{ ngVerticalBarVisible: true }\">&nbsp;</div>\r" +
    "\n" +
    "\t<div ng-cell></div>\r" +
    "\n" +
    "</div>"
